...
--Soluciones+
--Ej1 Muestra todas las competiciones celebradas en Estados Unidos. (juntando la tabla nacionalidad)
...
--Ej2 Muestra los eventos en la competicion del año 2023 con el nombre de la disciplina, el nombre de la compretucuon ordenados por la distancia de ladisciplina
...
select d.nombre, c.nombre, d.distancia from eventos e
    join disciplinas d on d.id = e.id_disciplina
    join competicion c on c.id = e.id_competicion
where c.fecha_inicio like '%2023%'
order by d.distancia asc;

--Ej3 cuantas eltrletas hay en cada pais, quedate solo con los paises que tienen dos o mas atrletas, ordenalos por cantidad
SELECT pais, COUNT(*) AS cantidad_atletas FROM atletas
GROUP BY pais HAVING COUNT(*) >= 2
ORDER BY cantidad_atletas DESC;

select distinct nombre, pais from altletas order by pais asc;



--Ej4 4.Muestra para cada carrera de los mundiales del 2023 el mejor tiempo y el peor tiempo
...
--Ej5 Para cada atleta muestra su nombre y su apellido en una única columna, 
--el tiempo obtenido y la diferencia de tiempo con el atleta que quedó por 
--delante para el evento con valor 2 . Descarta aquellos atletas que no tengan tiempo .

SELECT CONCAT(a.nombre, ' ', a.apellido) AS nombre_completo, r.resultado, 
...
--Ej6. Mostrar los nombres de las carreras femeninas de menos de 600 metros.
SELECT nombre FROM disciplinas
WHERE es_masculina = FALSE AND distancia < 600;

--Ej7. Mostrar todas las puntuaciones del atleta Noah Lies en todas las disciplinas
...
SELECT d.nombre AS "nombre",
       IFNULL(r.resultado, 'No compite') AS "tiempo",
       e.fecha
FROM disciplinas d
    ...
    LEFT JOIN atletas a ON r.id_atleta = a.id
        AND (a.nombre LIKE 'Noah')
        AND (a.apellido LIKE 'Lies')
WHERE d.es_masculina = TRUE
ORDER BY e.fecha;